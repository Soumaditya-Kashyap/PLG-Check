...
class PlagiarismService:
    ...
    def check_plagiarism(self, pdf_chunks: List[str], document_id: str) -> Dict[str, Any]:
        """Complete plagiarism check workflow"""
        try:
            ...
            # Generate search queries from PDF chunks
            logger.info("Generating search queries from PDF content...")
            search_queries = self._generate_search_queries(pdf_chunks)
            logger.info(f"Generated {len(search_queries)} search queries")
            
            # Search arXiv for academic papers
            logger.info("Searching arXiv for academic papers...")
            arxiv_papers = self.arxiv_service.search_multiple_queries(search_queries, max_results_per_query=3)
            ...
            web_results = self.tavily_service.search_multiple_queries(search_queries, max_results_per_query=2)
            ...
            similarity_results = self.find_similar_content(pdf_chunks)
            ...
            if matches:
                ...
                for i, match in enumerate(matches[:3], 1):
                    ...
                    title = match.get('source_title', 'No title')[:50]
